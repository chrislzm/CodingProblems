        ...
        for(int i=1; i <= longer.length(); i++) {
            ...
            for(int j=1; j <= shorter.length(); j++) {
                ...
                if(shorter.charAt(j-1) == longer.charAt(i-1)) {
                    ...
                    cost[j] = Math.min(cost[j-1] + shorter.charAt(j-1), iMinus1_j + longer.charAt(i-1));
                }
                iMinus1_jMinus1 = iMinus1_j; // Setup for next loop 
            }
            iMinus1_jMinus1 = cost[0]; // Setup for next loop
        }
