...
public class OutputSortedListings_v2 {
    ...
    public static Map<String,LinkedList<Listing>> getHostListingMap(ArrayList<String> listings) {
        Map<String,LinkedList<Listing>> output = new HashMap<String,LinkedList<Listing>>();
        for(int i=1; i<listings.size(); i++) {
            String listing = listings.get(i);
            String[] fields = listing.split(",");
            String hostId = fields[0];
            float score = Float.parseFloat(fields[2]);
            output.computeIfAbsent(hostId, k->new LinkedList<Listing>()).add(new Listing(score,hostId,listing));
        }
        return output;
    }

    private static void addNextListing(String hostId, Map<String,LinkedList<Listing>> hostListingMap, PriorityQueue<Listing> q) {
        LinkedList<Listing> nextListings = hostListingMap.get(hostId);
        if(!nextListings.isEmpty()) {
            q.add(nextListings.removeFirst());
        }
    }
    
    public static List<String> getSortedPages(ArrayList<String> listings, int perPage) {
        Map<String,LinkedList<Listing>> hostListingMap = getHostListingMap(listings);
        ...
        for(LinkedList<Listing> hostListings : hostListingMap.values()) {
            q.add(hostListings.removeFirst());
        }
        LinkedList<Listing> temp = new LinkedList<>();
        ...
        while(!q.isEmpty()) {
            ...
            if(uniqueHostsOnPage.contains(l.hostId)) {
                ...
                addNextListing(l.hostId,hostListingMap,q);
                if(++listingsOnPage == perPage) {
                    while(!temp.isEmpty()) {
                        q.add(temp.removeFirst());
                    }
                    listingsOnPage = 0;
